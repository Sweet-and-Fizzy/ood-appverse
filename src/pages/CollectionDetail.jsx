...
import { fetchAppsByCollection } from '../utils/api';
import ErrorMessage from '../components/common/ErrorMessage';
import LoadingSpinner from '../components/common/LoadingSpinner';
import DetailHeader from '../components/common/DetailHeader';
import CollectionIcon from '../components/common/CollectionIcon';
import CollectionTreeList from '../components/detail/CollectionTreeList';

export default function CollectionDetail() {
  const { slug } = useParams();
  const { collections, loading, error, refetch, getCollectionBySlug } = useAppverseData();
  const [searchParams, setSearchParams] = useSearchParams();
  const expandedAppSlug = searchParams.get('app');
  const track = useTracking();
  const config = useConfig();

  const collection = !loading ? getCollectionBySlug(slug) : null;
  ...
  useEffect(() => {
    ...
    setAppsLoading(true);
    fetchAppsByCollection(collection.id, config)
      .then(data => setApps(data))
      .catch(err => console.error('Failed to fetch apps for collection:', err))
      .finally(() => setAppsLoading(false));
  }, [collection?.id, config]);

  useEffect(() => {
    if (collection) {
      track('collection_view', { collection_slug: slug, collection_id: collection.id });
    }
  }, [collection, slug, track]);
    ...
    return (
      <div className="mx-6 mt-6">
        <p className="text-appverse-black font-sans">
          ...
          <a href="#/collections" className="text-appverse-red hover:underline">Back to Collections</a>.
        </p>
      </div>
    );
  }
  ...
  // flicker.
  const effectiveApps = apps.length ? apps : (collection.apps || []);
  const collectionWithFullApps = { ...collection, apps: effectiveApps };

  // AppRow uses `isExpanded` keyed on app.id (not slug). CollectionDetail
    ...
    <div className="mb-4 bg-white">
      <div className="mx-6 mt-6">
        <DetailHeader
          ...
          usingButtonLabel="Using Collections"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-8">
          <aside>
            {/* Stacked-tree icon — same motif as CollectionTile */}
            <div className="mb-4 flex justify-start">
              <CollectionIcon size={72} />
            </div>
            ...
            {/* WWW + DOCS icon-links */}
            {(collection.wwwUrl || collection.docsUrl) && (
              <div className="flex items-center gap-4 mb-4">
                {collection.wwwUrl && (
                  <a
                    href={collection.wwwUrl}
                    target="_blank"
            ...
            {collection.sharedPaths && collection.sharedPaths.length > 0 && (
              <div className="mt-6">
                ...
                <ul className="list-none p-0 m-0 text-sm font-sans text-appverse-black">
                  {collection.sharedPaths.map((path, i) => (
          ...
          <main>
            <CollectionTreeList
              collection={collectionWithFullApps}
