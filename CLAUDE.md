...
Both use HashRouter. The widget reads and syncs with the URL hash at `/#/*` paths. Hash routing avoids conflicts with server-side routing (e.g., Drupal).
...
| Path | Component |
|------|-----------|
| `/#/` | SoftwareHome - Grid with FilterSidebar, search syncs to URL params |
| `/#/:slug` | SoftwareDetail - Detail view with AppList (slug-based) |
| `/#/software/:id` | SoftwareDetail - Detail view with AppList (UUID-based, backward compat) |
